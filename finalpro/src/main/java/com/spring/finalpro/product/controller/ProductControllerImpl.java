...
public class ProductControllerImpl implements ProductController {
	...
	// ProductList.jsp // 상품 리스트 화면 출력 (카테고리 값 받아서 출력하도록)
		@Override
		...
		public ModelAndView listProducts(HttpServletRequest request, HttpServletResponse response) {
		    ...
		    // 가격 범위 설정 관련(문자열로 들어온 가격대 숫자로 바꾸기)
			...
			if(colorList != null && !colorList.isEmpty()) {
				for(int i=0; i<colorList.size(); i++) {
					String colorCode = colorList.get(i);// 예제 색상 코드 
					// RGB 값으로 변환
			...
			// mybatis selectProductList sql문에서 처리할 데이터값들 Map에 set
			...
			// 페이지 출력 전체 정보 db jsp에서 출력하기 위한 설정
	...
	// detailProduct.jsp // 색상 검색 상품 리스트 화면 출력
	@Override
	...
	// searchList.jsp // 색상 검색이 아닌 상품 검색 리스트 화면 출력
   @Override
   ...
   // searchDetailList.jsp // 상품 검색 상세 리트스 화면 출력
   @Override
